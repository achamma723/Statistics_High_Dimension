...
def comput_statistic(X, y, i, test_size=0.2, n_repeats=n_repeats):
...
list_z_stat = parallel(delayed(comput_statistic)(X, y, i, test_size=0.2, n_repeats=n_repeats)
    for i in tqdm(range(n_trials)))
t = time.time() - start
print(f"Time elapsed:{t}")

# Plot the histogram
plt.hist(list_z_stat, density=True)


plt.title("NO Conditional permutation \n Parameters:n="+str(n)+"/p="+str(p)+"/n_signal="+str(n_signal)
